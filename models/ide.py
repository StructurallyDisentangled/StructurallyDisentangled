...
def features_normalization(features, eps=torch.tensor(torch.finfo(torch.float32).eps)):
    """Normalize the features to be betwen -1 and 1"""
    features= features / torch.maximum((torch.max(features, dim=-1, keepdim=True)[0] - torch.min(features, dim=-1, keepdim=True)[0]),eps)
    features = 2 * features - 1

    # normalize features magnitude to be 1 (values are -1,1)
...
def generate_ide_fn(deg_view):
    ...
    return integrated_dir_enc_fn, 2 * ml_array.shape[1]
