...
from datasets.utils.pyramid_embedding_dataloader import PyramidEmbeddingDataloader 
...
def create_spheric_poses(cameras, n_steps=120,pos_mult=1.):
    ...
    for theta in torch.linspace(0, 2 * math.pi, n_steps):
        ...
        c2w = torch.cat([torch.stack([s, u, -l], dim=1), cam_pos[:,None]], axis=1)*pos_mult
        all_c2w.append(c2w)
...
class ColmapDatasetBase():
    ...
    def setup(self, config, split):
        ...
        if not ColmapDatasetBase.initialized:
            ...
            for i, d in enumerate(imdata.values()):
                ...
                if self.split in ['train', 'val']:
                    # if d.name.endswith('JPG'):
                    #     img_path = os.path.join(self.config.root_dir, 'images', d.name.split('.')[0] + '.jpg')    
                    # else:                        
                    ...
                    if self.config.load_features:
                        feat = torch.load(os.path.join(self.config.root_dir, 
                                                       f'features_{self.config.img_downscale}', 
                                                       d.name.split('.')[0] + '_feat.pt')).to('cpu')
        ...
        if self.split == 'test':
            self.all_c2w = create_spheric_poses(self.all_c2w[:,:,3], n_steps=self.config.n_test_traj_steps, pos_mult=self.config.get('traj_pos_mult',1.)) # [151, 3, 4]
            # self.all_c2w = self.all_c2w[::8] # [151, 3, 4]
            ...
            # self.all_c2w = self.all_c2w[::8]
            # self.all_images, self.all_fg_masks = torch.stack(self.all_images[::8], dim=0).float(), torch.stack(self.all_fg_masks[::8], dim=0).float()
            # self.all_points = torch.tensor([])
            # self.all_points_confidence = torch.tensor([])
            # self.all_fg_indexs, self.all_bg_indexs = torch.tensor([]), torch.tensor([])
        else:
        ...
        if "lerf_config" in self.config:
            self.lerf_config = self.config.lerf_config
            self.clip_model = OpenCLIPNetwork(self.lerf_config.clip_model) # clip model
            clip_cache_path = Path(osp.join(self.config.root_dir, f"clip_{self.clip_model.name}_{self.config.img_downscale}"))
            
            self.clip_interpolator = PyramidEmbeddingDataloader(
                image_list=self.all_images,
                device=self.all_images[0].device,
                cfg={
                    "tile_size_range": [0.05, 0.5],
                    "tile_size_res": 7,
                    "stride_scaler": 0.5,
                    "image_shape": [self.img_wh[-1], self.img_wh[-2]],
                    "model_name": self.clip_model.name,
                },
