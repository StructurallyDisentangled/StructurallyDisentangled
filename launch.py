...
def main():
    ...
    parser.add_argument('--seg_total', action='store_true')
    parser.add_argument('--disable_edit', action='store_true')

    parser.add_argument('--exp_dir', default='./exp')
    parser.add_argument('--runs_dir', default='./runs')
    parser.add_argument('--verbose', action='store_true', help='if true, set logging level to DEBUG')

    parser.add_argument('--trial_name', default=None, help='name of the trial')
    ...
    config.trial_name = config.get('trial_name') or (args.trial_name) or (config.tag + datetime.now().strftime('@%Y%m%d-%H%M%S'))
    ...
    if args.seg_total and config.model.get('edit_config',None) is not None:
        for segment in config.model.edit_config.segments:
            segment.seg_type = 'total'

    if args.disable_edit:
        # remove config.model.edit_config
        config.model.pop('edit_config', None)

    # if not args.train:
    #     config.model.geometry.xyz_encoding_config.start_level = config.model.geometry.xyz_encoding_config.n_levels
    #     config.model.geometry_bg.xyz_encoding_config.start_level = config.model.geometry_bg.xyz_encoding_config.n_levels
    ...
    if (not args.train) and \
        config.model.geometry.xyz_encoding_config.otype == 'ProgressiveBandHashGrid':
        # Update level of ProgressiveBandHashGrid if not training
        config.model.geometry.xyz_encoding_config.start_level = config.model.geometry.xyz_encoding_config.n_levels
    
    if (not args.train) and \
        config.model.geometry_bg.xyz_encoding_config.otype == 'ProgressiveBandHashGrid':
        # Update level of ProgressiveBandHashGrid if not training
        config.model.geometry_bg.xyz_encoding_config.start_level = config.model.geometry_bg.xyz_encoding_config.n_levels
