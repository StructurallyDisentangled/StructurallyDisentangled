...
class BaseSystem(pl.LightningModule, SaverMixin):
    ...
    def __init__(self, config):
        ...
        if self.config.get('model_2', None) is not None:
            self.model_2 = models.make(self.config.model_2.name, self.config.model_2)
    ...
    def on_train_batch_start(self, batch, batch_idx, unused=0):
        ...
        if self.config.get('model_2', None) is not None:
            update_module_step(self.model_2, self.current_epoch, self.global_step)
    ...
    def configure_optimizers(self):
        if self.config.system.get('selected_model', 0) == 1:
            optim = parse_optimizer(self.config.system.optimizer, self.model_2)
        else:
